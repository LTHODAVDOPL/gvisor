...
func readFromDisk(dev io.ReadSeeker, abOff int64, v interface{}) error {
	_, err := dev.Seek(abOff, io.SeekStart)
	if err != nil {
		return syserror.EIO
	}

	err = binary.Read(dev, binary.LittleEndian, v)
	if err != nil {
		return syserror.EIO
	}
...
func readSuperBlock(dev io.ReadSeeker) (disklayout.SuperBlock, error) {
	...
	err := readFromDisk(dev, disklayout.SbOffset, sb)
	if err != nil || sb.Revision() == disklayout.OldRev {
		return sb, err
	}

	sb = &disklayout.SuperBlock32Bit{}
	err = readFromDisk(dev, disklayout.SbOffset, sb)
	if err != nil || !sb.IncompatibleFeatures().Is64Bit {
		return sb, err
	}
...
func readBlockGroups(dev io.ReadSeeker, sb disklayout.SuperBlock) ([]disklayout.BlockGroup, error) {
	...
	var bgds []disklayout.BlockGroup = make([]disklayout.BlockGroup, bgCount)
	...
	for i, off := uint64(0), uint64(sb.FirstDataBlock()+1)*sb.BlockSize(); i < bgCount; i, off = i+1, off+bgdSize {
		...
		err := readFromDisk(dev, int64(off), bgds[i])
		if err != nil {
			return nil, err
		}
	}
